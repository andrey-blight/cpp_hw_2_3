...
void fill_velocity(int start, int end, Fixed g) {
...
Fixed apply_forces_p(int start, int end) {
    Fixed total_delta = 0;
...
int main() {
    ...
    for (size_t i = 0; i < T; ++i) {
        ...
        for (int j = 0; j < num_threads; ++j) {
            ...
            int end = min(N, (size_t) start + chunk_size);
            threads.emplace_back(fill_velocity, start, end, g);
        }
        ...
        vector<std::future<Fixed>> futures;

        for (int j = 0; j < num_threads; ++j) {
            int start = j * chunk_size;
            int end = min(N, (size_t) start + chunk_size);
            futures.emplace_back(std::async(std::launch::async, apply_forces_p, start, end));
        }
